...
class TestRepositoryDetectionViewSet(APIBaseTest):
    ...
    def test_list_filters_by_repository_and_kind(self):
        self.client.post(self._url(), self._payload(), format="json")
        self.client.post(self._url(), self._payload(repository="posthog/wizard"), format="json")

        self._authenticate_personal_api_key(["wizard_session:read"])
        response = self.client.get(self._url(), {"repository": "posthog/wizard", "kind": "error-tracking-source-maps"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        results = response.json()["results"]
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["repository"], "posthog/wizard")
