...
class SetupWizardSourcemapDetectionSerializer(serializers.Serializer):
    project_id = serializers.IntegerField(
...
class SetupWizardViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        request=SetupWizardSourcemapDetectionSerializer,
        responses={200: SetupWizardCloudRunResponseSerializer},
    )
    @action(
        methods=["POST"],
        detail=False,
        url_path="sourcemaps_detection",
        authentication_classes=[SessionAuthentication],
        permission_classes=[IsAuthenticated],
        throttle_classes=[
            SetupWizardCloudRunBurstRateThrottle,
    ...
    def sourcemaps_detection(self, request: Request) -> Response:
        """Scan a GitHub repository for source-map setup, in the cloud.

        Provisions a task-run sandbox that clones the repository and runs the wizard's source-map
        detection headlessly. The wizard posts the resulting report to the wizard product's
        repository-detections API; the app reads it from there later. No agent runs and no pull
        request is opened. Shares the cloud wizard run's per-user daily attempt budget because
        each run starts a sandbox.
        """
        if not bool(settings.WIZARD_CLOUD_RUN_OAUTH_CLIENT_ID):
            raise exceptions.NotFound("Running the setup wizard in the cloud is not available.")

        serializer = SetupWizardSourcemapDetectionSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        try:
            result = tasks_facade.create_sourcemap_detection_run(
                team=project.passthrough_team,
                user_id=cast(User, request.user).id,
                repository=serializer.validated_data["repository"],
            )
        except ValueError as e:
            # e.g. the team has no GitHub integration with access to the repository.
            raise exceptions.ValidationError(str(e))
